...
class NLIConsistency(Estimator):
    def __init__(
        self,
    ):
        super().__init__(["greedy_semantic_matrix_entail"], "sequence")

    def __str__(self):
        return "NLIConsistency"

    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        batch_greedy_sentence_similarity = stats["greedy_semantic_matrix_entail"]
...
class NLICocoaMSP(Estimator):
    def __init__(
        self,
    ):
        super().__init__(
            ["greedy_semantic_matrix_entail", "greedy_log_likelihoods"], "sequence"
        )
...
class NLICocoaMTE(Estimator):
    def __init__(self):
        super().__init__(["greedy_semantic_matrix_entail", "mean_entropy"], "sequence")

    def __str__(self):
        return "NLICocoaMTE"

    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        batch_mean_entropy = stats["mean_entropy"]
        batch_greedy_sentence_similarity = stats["greedy_semantic_matrix_entail"]

        enriched_entropy = []

        for mean_entropy, greedy_sentence_similarity in zip(
            batch_mean_entropy, batch_greedy_sentence_similarity
        ):
            # Compute average dissimilarity
            avg_dissimilarity = np.mean(1 - greedy_sentence_similarity)

            enriched_value = mean_entropy * avg_dissimilarity
            enriched_entropy.append(enriched_value)
